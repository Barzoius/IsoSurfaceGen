...
[CreateAssetMenu(fileName = "DualContouringGenerator", menuName = "MeshGenerators/DualContouring")]
public class DCGenerator : MeshGenerator
{
        ...
        for (int x = 0; x <= gridSize; x++)
        {
            for (int y = 0; y <= gridSize; y++)
            {
                for (int z = 0; z <= gridSize; z++)
                {
                    ...
                    if (cnt != 0)
                    {
                        qef.solve(out Vector3 qefPosition, 1e-3f, 16, 1e-3f);
